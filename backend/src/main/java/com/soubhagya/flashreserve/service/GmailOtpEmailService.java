...
public class GmailOtpEmailService implements EmailService {
	...
	public void sendOtpEmail(String toEmail, String otp) {
		String subject = "FlashReserve email verification code";
		String expiryText = formatExpiry();
		String from = fromAddress != null && !fromAddress.isBlank() ? fromAddress : "no-reply@flashreserve.local";
		String html = buildHtml(otp, expiryText);
		String text = buildText(otp, expiryText);
		try {
	...
	private String buildHtml(String otp, String expiryText) {
		return """
				<div style="font-family: system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif; max-width:520px; margin:0 auto; padding:24px;">
				  <div style="border:1px solid #F3D1DE; border-radius:18px; padding:28px; background:#FFF7FB;">
				    ...
				    <h2 style="margin:0 0 8px; font-size:18px; color:#241333;">Your verification code</h2>
				    <p style="margin:0 0 16px; color:#6B587A; font-size:14px;">Use this code to verify your email and finish creating your account.</p>
				...
				""".formatted(escapeHtml(otp), escapeHtml(expiryText));
	}

	private String buildText(String otp, String expiryText) {
		return """
				FlashReserve
